...
impl DPLLSolve for Clauses {
    fn dpll(&self) -> bool {
        ...
        let clauses_left: Clauses = self.iter().filter(|clause| !clause.contains(&literal)).cloned().collect();
        let clauses_right: Clauses = self.iter().filter(|clause| !clause.contains(&literal.negate())).cloned().collect();
...
impl DPLLHeuristics for Clauses {
    fn unit_propagate(&self) -> Clauses {
        ...
        loop {
            let unit_clauses: Clauses = clauses.iter().filter(|&clause| clause.len() == 1).cloned().collect();
    ...
    fn pure_literal_elimination(&self) -> Clauses {
        ...
        let all_literals: HashSet<_> = clauses.iter()
            .flat_map(|clause| clause.iter())
            .cloned()
            .collect();

        let pure_literals: HashSet<_> = all_literals.iter()
